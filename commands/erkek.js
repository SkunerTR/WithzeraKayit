...
exports.run = async (client, message, args) => {
    ...
    */
    try {
        ...
        if (!piece) return message.channel.send(new Discord.MessageEmbed().setDescription(`Bir kullanıcı belirtmelisin. **Örnek: @Miaf/324886053884264449**`).setFooter(`Withzera - Kayıt Sistemi`).setColor("RANDOM").setTimestamp());
        if (!miaf) return message.channel.send(new Discord.MessageEmbed().setDescription(`İsim belirtmelisin. **Örnek: Miaf**`).setFooter(`Withzera - Kayıt Sistemi`).setColor("RANDOM").setTimestamp());
        if (!serendia) return message.channel.send(new Discord.MessageEmbed().setDescription(`Yaş belirtmelisin. **Örnek: 17**`).setFooter(`Withzera - Kayıt Sistemi`).setColor("RANDOM").setTimestamp());

        message.channel.send(new Discord.MessageEmbed().setDescription(`**__Kayıt İşlemi Başarılı__**\n\n🤠 Kayıt Edilen Kişi: ${piece}\n🤠 Kayıt Yapan Yetkili: ${message.author}\n🤠 Kayıt İşleminde Verilen Rol: <@&${config.erkekRol}>\n🤠 Kayıt İşleminde Alınan Rol: <@&${config.kayitsiz}>`).setFooter(`Withzera - Kayıt Sistemi`).setColor("RANDOM").setTimestamp())
